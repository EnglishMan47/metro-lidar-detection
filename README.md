...
Рабочий план и ежедневная нагрузка: [PROJECT_PLAN.md](PROJECT_PLAN.md). Подтверждённые требования: [docs/REQUIREMENTS.md](docs/REQUIREMENTS.md). Аудит данных: [docs/DATA_REPORT.md](docs/DATA_REPORT.md). Разметка положительного bag: [docs/POSITIVE_BAG_ANNOTATION.md](docs/POSITIVE_BAG_ANNOTATION.md). Конспект Q&A: [docs/QNA_NOTES.md](docs/QNA_NOTES.md). Среда запуска: [docs/ENVIRONMENT.md](docs/ENVIRONMENT.md). Инструкция GitHub: [docs/GITHUB_SYNC.md](docs/GITHUB_SYNC.md). Текущее состояние: [docs/STATUS.md](docs/STATUS.md).
...
Воспроизводимая предварительная разметка положительного bag:

```powershell
.\.venv\Scripts\python.exe scripts\annotate_positive_bag.py data\extracted\for_hackathon\doubleT_obstacle\doubleT_obstacle_0.db3 --output annotations\doubleT_obstacle.json
```
